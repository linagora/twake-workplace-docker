...
export default function AdminConfigPage() {
  ...
  async function fetchConfig() {
    ...
    setError('')
    try {
      const data = await apiFetch<{ data?: { services?: Record<string, any> }; services?: Record<string, any> }>(
        '/admin/config', { headers: authHeaders() }
      )
      // Handle both { services: {...} } and { data: { services: {...} } }
      const raw: Record<string, any> = (data as any).services ?? (data as any).data?.services ?? {}
      const parsed: ServiceConfig[] = Object.entries(raw).map(([name, cfg]) => ({
        name,
  ...
  function updateService(name: string, field: keyof ServiceConfig, value: string | boolean) {

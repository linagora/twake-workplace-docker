...
export const SERVICES = [
  ...
  { id: 'twake-calendar', label: 'Calendar CalDAV', endpoint: 'https://tcalendar-side-service.twake.local', scope: 'openid email profile',
    curlExample: (token, aid, user) => `# List calendars for ${user || 'user'}\ncurl -sk -X PROPFIND ${PROXY}/twake-calendar/dav/calendars/${user || 'USER'}/ \\\n  -H "Authorization: Bearer ${token}" \\\n  -H "Depth: 1" \\\n  -H "Content-Type: application/xml" \\\n  -d '<?xml version="1.0"?><d:propfind xmlns:d="DAV:"><d:prop><d:displayname/><d:resourcetype/></d:prop></d:propfind>'` },
  { id: 'twake-chat', label: 'Matrix Chat', endpoint: 'https://matrix.twake.local/_matrix/client/v3', scope: 'm.room.message',
    curlExample: (token) => `# List joined rooms\ncurl -sk ${PROXY}/twake-chat/_matrix/client/v3/joined_rooms \\\n  -H "Authorization: Bearer ${token}"` },
  { id: 'twake-drive', label: 'Cozy Drive', endpoint: 'https://user1.twake.local/files/', scope: 'io.cozy.files',
    curlExample: (token, aid, user) => `# List root directory files\ncurl -sk ${PROXY}/twake-drive/data/io.cozy.files/io.cozy.files.root-dir \\\n  -H "Authorization: Bearer ${token}" \\\n  -H "Accept: application/vnd.api+json"` },
]

export default function CreateTokenDialog({ open, onClose, onCreated, existingTokens = [] }) {
  // Services that have an active service token
  const activeServiceIds = existingTokens
    .filter(t => t.type === 'service' && t.status === 'ACTIVE')
    .map(t => t.service)
  const hasAnyServiceToken = activeServiceIds.length > 0
  ...
  const [displayTab, setDisplayTab] = useState('token')
  ...
  async function handleCreate() {
    ...
    try {
      if (tokenType === 'service') {
        ...
        const data = await apiFetch('/umbrella-token', {
          method: 'POST',
          headers,
            ...
            {tokenType === 'service' ? (
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {SERVICES.map(s => {
                    const hasToken = activeServiceIds.includes(s.id)
                    return (
                      <label key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: hasToken ? 'pointer' : 'default', fontSize: 14, color: hasToken ? '#1a1a2e' : '#c0c5ce' }}>
                        <input
                          type="checkbox"
                          checked={selectedScopes.includes(s.id)}
                          onChange={() => hasToken && toggleScope(s.id)}
                          disabled={!hasToken}
                          style={{ accentColor: '#297EF2' }}
                        />
                        {s.label}
                        {!hasToken && <span style={{ fontSize: 11, color: '#c0c5ce', fontStyle: 'italic' }}>(no service token)</span>}
                      </label>
                    )
                  })}
                </div>
          ...
          <div style={bodyStyle}>
            ...
            {displayTab === 'usage' && (() => {
              ...
              if (isUmbrella) {
                ...
                const allCurls = scopes.map(s => {
                  const svc = SERVICES.find(x => x.id === s)
                  return svc ? `# ${svc.label}\n${svc.curlExample(tkn, aid, username)}` : `# ${s}\ncurl -sk ${proxyBase}/${s}/ \\\n  -H "Authorization: Bearer ${tkn}"`
                })
                curlCmd = `# Umbrella token — use via Token Manager proxy\n\n${allCurls.join('\n\n')}`
              } else {
                const username = getCurrentUserEmail().split('@')[0]
                curlCmd = svc?.curlExample?.(tkn, jmapAccountId || 'YOUR_ACCOUNT_ID', username) ?? ''
              }

...
const SERVICES = [
  { id: 'twake-mail', label: 'TMail JMAP', endpoint: 'https://jmap.twake.local/jmap', scope: 'openid email profile', curlExample: (token) => `curl -sk -X POST https://jmap.twake.local/jmap \\\n  -H "Authorization: Bearer ${token}" \\\n  -H "Content-Type: application/json" \\\n  -d '{"using":["urn:ietf:params:jmap:core","urn:ietf:params:jmap:mail"],"methodCalls":[["Mailbox/get",{"accountId":"YOUR_ACCOUNT_ID","ids":null},"c1"]]}'` },
  { id: 'twake-calendar', label: 'Calendar CalDAV', endpoint: 'https://tcalendar-side-service.twake.local', scope: 'openid email profile', curlExample: (token) => `curl -sk -X PROPFIND https://tcalendar-side-service.twake.local/dav/principals/ \\\n  -H "Authorization: Bearer ${token}" \\\n  -H "Depth: 0"` },
  { id: 'twake-chat', label: 'Matrix Chat', endpoint: 'https://matrix.twake.local/_matrix/client/v3', scope: 'm.room.message', curlExample: (token) => `curl -sk https://matrix.twake.local/_matrix/client/v3/joined_rooms \\\n  -H "Authorization: Bearer ${token}"` },
  { id: 'twake-drive', label: 'Cozy Drive', endpoint: 'https://user1.twake.local/files/', scope: 'io.cozy.files', curlExample: (token) => `curl -sk https://user1.twake.local/files/io.cozy.files.root-dir \\\n  -H "Authorization: Bearer ${token}" \\\n  -H "Accept: application/vnd.api+json"` },
]

export default function CreateTokenDialog({ open, onClose, onCreated }) {
    ...
    try {
      if (tokenType === 'service') {
        ...
        const resp = await fetch(
          `${window.location.origin}/api/v1/umbrella-token`,
          {
            method: 'POST',
            headers,
